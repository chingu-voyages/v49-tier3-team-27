  ...
  return (
    <section className="w-full flex flex-col gap-2 p-3">
        <div className="w-full flex flex-row gap-3 justify-around max-sm:overflow-hidden max-sm:overflow-x-auto">
            {labels.map(category => (
                <button type="button" onClick={() => updateActiveCategory(category.slug)} key={`events-category-${category.name}`} className={`flex flex-col gap-1 items-center justify-between hover:bg-figma-brown hover:bg-opacity-10 p-2 rounded-sm transition-colors duration-300 ${activeCategory == category.slug && " bg-figma-brown bg-opacity-10"}`}>
                    <Image 
                        src={category.icon}
                        alt=""
                        width={39}

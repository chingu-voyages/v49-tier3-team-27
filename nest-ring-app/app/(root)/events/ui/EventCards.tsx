...
const EventCards = () => {
  const { isLoading, events, activeCategory } = useContext(EventsTabContext);
    ...
    <section className="w-full bg-white mt-3 px-10 flex flex-row flex-wrap max-md:justify-center gap-2 ">
      {isLoading ? (
        ...
        <>
          ...
          {events.length < 1 && (
            <div className="w-full mt-4 flex flex-col items-center justify-center font-light font-mono">
              <span className="text-lg text-figma-brown">
                You have no events to view
              </span>
              <Link
                href={"/events/create"}
                className=" w-[100px] rounded-md border text-interactive-green hover:text-interactive-green text-center"
              >
                create one
              </Link>
            </div>

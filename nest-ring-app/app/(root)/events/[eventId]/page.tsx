...
const EventViewPage = ({
  params: { eventId },
}: Readonly<{ params: { eventId: string } }>) => {
  const { events } = useContext(EventsTabContext);
  const [activeEvent, setActiveEvent] = useState<EventsObjType | null>(null);

  useEffect(() => {
    const actEvt = events.find((event) => event._id == eventId);
    actEvt && setActiveEvent(actEvt);
  }, [events]);

  return (
    <main className=" w-full max-md:h-[91%] h-full max-md:mb-12 bg-white rounded-t-lg p-4 overflow-hidden overflow-y-auto">
      {activeEvent ? (
        <div className="w-full h-full flex flex-col items-center gap-2 sm:p-5">
          <div className="w-full sm:pl-5 flex flex-row justify-start">
            <PrevPageBtn />
          </div>
          <span className=" text-center font-bold font-serif text-4xl text-figma-brown">
            {nameMap.find((ent) => ent.eventType == activeEvent.category)
              ?.eventName || "Event"}
          </span>
          <span className=" font-bold text-figma-brown text-xl text-center">
            {activeEvent.subject}
          </span>
        ...
        <span className="font-bold text-sm">
          Event not found; Check again later.
        </span>
      )}
    </main>
  );
